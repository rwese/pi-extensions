/**
 * TUI components and the `subagents:config` command.
 *
 * `ui/ui-config.ts` is the topmost layer in the subagent
 * extension: it owns the visible UI surface (the tool-toggle
 * picker and the agent-select picker) plus the
 * `subagents:config` command that wires them together.
 *
 * Dependencies are restricted to the `core/` layer
 * (`core/settings` for the persisted config, `core/types` for
 * the persisted agent-config shape) and `@earendil-works/pi-tui`
 * for the input / render primitives. No I/O happens in this
 * module; reads/writes go through `core/settings`.
 */
...
import type { SubagentSettings } from "../agents.js";

/**
 * A two-line checkbox list with `S` to save and `Esc` to
 * cancel. `S` invokes `onDone(selectedNames)`;
 * `Esc` invokes `onCancel()`. `onDone`/`onCancel` are set by
 * the caller; the class is otherwise stateless w.r.t. the
 * outside world.
 *
 * The instance caches its rendered lines per `width`; call
 * `invalidate()` after a mutation to drop the cache.
 */
...
/**
 * Register the `subagents:config` command on `pi`. The command
 * opens a two-step TUI:
 *
 *   1. Agent pick: shows the user-scope agent roster with the
 *      current tool summary (`defaults` vs an explicit list).
 *   2. Tool toggle: shows a `ToolToggleList` for the picked
 *      agent. Pressing `S` saves the override; pressing `Esc`
 *      loops back to the agent picker.
 *
 * The save step drops the `tools` override if the chosen set
 * matches the agent's built-in / frontmatter defaults so we do
 * not persist a no-op override. Other fields (model,
 * timeoutMs) are preserved across the save.
 */
